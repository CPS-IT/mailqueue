...
{
    private const DEFAULT_WAIT_INTERVAL = 5;
    ...
    protected function configure(): void
    {
        ...
        $this->addOption(
            ...
            sprintf(
                'Watch for changes within mail queue, e.g. if mails are sent (by default, refreshes all %d seconds)',
    ...
    protected function execute(Console\Input\InputInterface $input, Console\Output\OutputInterface $output): int
    {
        ...
        $watch = (int)($input->getOption('watch') ?? self::DEFAULT_WAIT_INTERVAL);

        // Early return if --watch option is used on an unsupported output
        if ($watch > 0 && !($output instanceof Console\Output\ConsoleOutput)) {
        ...
        if ($watch > 0) {
            ...
            $this->watchForQueueUpdates($section, $transport, $watch);
        } else {
    ...
    private function watchForQueueUpdates(
        ...
        while (true) {
            ...
            $this->io->writeln(
                sprintf(
                    '<fg=gray>List is refreshed every %s. Exit with Ctrl+C.</>',
                    $waitInterval === 1 ? 'second' : $waitInterval . ' seconds',
                ),
            );
